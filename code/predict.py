...
character_start_width = 300 
character_start_height = 100 
character_detection = []
bound = 40
...
def build_animation(character):
    background_reader = cv2.VideoCapture("dataVideo/badland.mp4")

    background_nb_frames = int(background_reader.get(cv2.CAP_PROP_FRAME_COUNT))
    background_frame_h = int(background_reader.get(cv2.CAP_PROP_FRAME_HEIGHT))
    background_frame_w = int(background_reader.get(cv2.CAP_PROP_FRAME_WIDTH))

    video_writer = cv2.VideoWriter("outputVideo/output.mp4",
                            cv2.VideoWriter_fourcc(*'MPEG'), 
                            50.0, 
                            (background_frame_w, background_frame_h))
                            # (150, 150))
    ...
    start_point = 0 #%
    show_window = False
    for i in tqdm(range(background_nb_frames)):
        _, image = background_reader.read()

        if (float(i+1)/background_nb_frames) > start_point/100.:
            images += [image]

            if (i%batch_size == 0) or (i == (background_nb_frames-1) and len(images) > 0):
        ...
        for w in range(character[i].shape[0]):
            for h in range(character[i].shape[1]):
                if(not (character[i][w,h][0] < bound and character[i][w,h][1] < bound and character[i][w,h][2] < bound )):
                    images_background[i][character_start_width+w,character_start_height + h] = character[i][w,h]
        video_writer.write(images_background[i]) 
        video_writer2.write(character[i]) 
    if show_window: cv2.destroyAllWindows()
    background_reader.release()
